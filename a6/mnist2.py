...
def conv_layer(input, size_in, size_out, name="conv"):
    with tf.name_scope(name):
        ...
        # get the weights into an image
        wb = w - b
        x_min = tf.reduce_min(wb)
        x_max = tf.reduce_max(wb)
        kernel_0_to_1 = (wb - x_min) / (x_max - x_min)
        kernel_transposed = tf.transpose(kernel_0_to_1, [3, 0, 1, 2])
        for kt in range(kernel_transposed.shape[3]):
            tf.summary.image('filter' + str(kt), kernel_transposed[:,:,:,kt:kt+1])
...
def mnist_model(learning_rate, use_two_fc, use_two_conv, hparam):
    ...
    if use_two_fc:
        ...
        embedding_size = 1024
        dropout = tf.layers.dropout(inputs=relu, rate=prob, training=False)
        logits = fc_layer(relu, 1024, 10, "fc2")
    else:
        embedding_input = pool2_flat
        embedding_size = 7*7*64
        dropout = tf.layers.dropout(inputs=pool2_flat, rate=prob, training=False)
        logits = fc_layer(pool2_flat, 7*7*64, 10, "fc")
    ...
    twriter = tf.summary.FileWriter(LOGDIR + hparam + '/test')
    ...
    for i in range(2001):
        ...
        if i % 5 == 0:
            [train_accuracy, s] = sess.run([accuracy, summ], feed_dict={x: batch[0], y: batch[1], prob: 0.4})
